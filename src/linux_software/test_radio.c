...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    unsigned int bytes_transferred = 2048*4; // 4bytes (32bits) per read operation
    float time_spent = (stop_time-start_time)*8e-9; // 8ns per clock cycle
    throughput = (bytes_transferred/time_spent)*1e-6; // (bytes/second)*(1e-6megabytes/byte)
    printf("You transferred %u bytes of data in %e seconds\n",bytes_transferred,time_spent);
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 Adam Bagley - Custom Peripheral Demonstration\n\r");
